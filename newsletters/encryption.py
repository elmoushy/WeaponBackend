"""
Encryption utilities for newsletters data using AES-256.

This module provides encryption/decryption functionality for newsletter data
using the same encryption standards as surveys and internal_chat.
"""
...
class NewslettersDataEncryption:
    """
    Encryption handler for newsletters data using AES-256 via Fernet.
    """
    ...
    def __init__(self):
        ...
        if not self.cipher_suite:
            logger.warning("Newsletters encryption not initialized - key not found")
    
    def _get_encryption_key(self):
        """Get encryption key from environment or generate one."""
        # Try newsletters-specific key first
        key = os.getenv('NEWSLETTERS_ENCRYPTION_KEY')
        
        if not key:
            # Fallback to surveys encryption key
...
# Global instance
newsletters_data_encryption = NewslettersDataEncryption()
